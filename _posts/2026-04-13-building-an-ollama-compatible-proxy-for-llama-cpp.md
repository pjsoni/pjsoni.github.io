...
date: 2026-04-15
category: [blog]
tags: [llama.cpp, ollama, ai, infrastructure, golang]
description: "How I bridged the gap between Ollama-based tools and llama.cpp using a lightweight Go proxy."
excerpt: A lightweight Go proxy that translates Ollama API requests into llama.cpp calls, allowing existing Ollama‑compatible tools to run on llama.cpp without code changes. The proxy handles chat, embeddings, and model discovery, while preserving performance tuning knobs. It bridges the gap between the two ecosystems, simplifying local LLM deployment. Efficiently.
